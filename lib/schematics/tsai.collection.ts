...
export class TsaiCollection extends AbstractCollection {
  ...
  constructor(runner: AbstractRunner) {
    super('@tsailab/schematics', runner);
  }
  ...
  public getSchematics(): Schematic[] {
    return TsaiCollection.schematics.filter(
      (item) => item.name !== 'angular-app',
    );
  }

  private validate(name: string) {
    const schematic = TsaiCollection.schematics.find(
